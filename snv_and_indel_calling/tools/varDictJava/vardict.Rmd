...
Fast indel and SNV caller for germline and/or somatic mutations.
...
CommandLineBinding(position = 201, prefix = ">", valueFrom = list('{return $job.inputs.bam[0].metadata.sample_id + $job.inputs.inputs.bed.metadata.name + "_vardict_variants.txt"}'))
)
```
...
inheritMetadataFrom = "#bam", metadata = list(from_tool = "VarDict", 
temp_name = Expression(engine = "#cwl-js-engine",
script = '{return $job.inputs.bam[0].metadata.sample_id + $job.inputs.inputs.bed.metadata.name + "_vardict_variants.txt"')),
...
write(tool$toJSON(pretty = TRUE), "../apps/vardict.json")
```

####push app to cloud platform

```{r eval=platform_push=="yes"}
project$app_add("vardict", tool)
```

####notes

This is how to make 5e6 MB bed files.
